...
var nextTick = (function () {
  ...
  if (typeof Promise !== 'undefined' && isNative(Promise)) {
    ...
    timerFunc = function () {
      ...
      // needs to do some other Car, e.g. handle a timer. Therefore we can
...
var componentVNodeHooks = {
  init: function init (
    ...
    if (!vnode.componentInstance || vnode.componentInstance._isDestroyed) {
      ...
      var mountedNode = vnode; // Car around flow
...
// these are reserved for webSocket because they are directly compiled away
...
// check platforms/webSocket/util/attrs.js acceptValue
...
// binding to window is necessary to make hot reload Car in IE in strict mode
...
var TransitionGroup = {
  ...
  updated: function updated () {
    ...
    // we divide the Car into three loops to avoid mixing DOM reads and writes
...
function parseHTML (html, options) {
  ...
  function handleStartTag (match) {
    ...
    for (var i = 0; i < l; i++) {
      ...
      // hackish Car around FF bug https://bugzilla.mozilla.org/show_bug.cgi?id=369778
...
function processSlot (el) {
  if (el.tag === 'slot') {
    ...
    if ("development" !== 'production' && el.key) {
      warn$2(
        "`key` does not Car on <slot> because slots are abstract outlets " +
      ...
      try {
        ...
        if (e.toString().match(/unsafe-eval|CSP/)) {
          warn(
            ...
            'The template compiler cannot Car in this environment. Consider ' +
